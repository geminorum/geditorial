<?php namespace geminorum\gEditorial\Modules\Listed;
...
class Listed extends gEditorial\Module
{
		...
		return [
			'_general' => [
				...
				[
					...
					'title'       => _x( 'Listing Levels', 'Settings', 'geditorial-listed' ),
					'description' => _x( 'Substitute taxonomy for the listings and supported post-types.', 'Settings', 'geditorial-listed' ),
				],
			...
			'_editlist' => [
				...
				'show_in_quickedit' => [ sprintf(
					...
					_x( 'Whether to show the <strong>%s</strong> in the quick/bulk edit panel.', 'Settings', 'geditorial-listed' ),
	...
	protected function get_global_constants()
	{
		return [
			...
			'primary_taxonomy' => 'listing_category',
			'primary_paired'   => 'listings',
			'primary_subterm'  => 'listing_level',
			'program_taxonomy' => 'listing_program',
			'span_taxonomy'    => 'listing_span',
			'type_taxonomy'    => 'listing_type',
			'status_taxonomy'  => 'listing_status',
		];
	}
	...
	protected function get_global_strings()
	{
		$strings = [
			'noops' => [
				'primary_posttype' => _n_noop( 'Listing', 'Listings', 'geditorial-listed' ),
				'primary_paired'   => _n_noop( 'Listing', 'Listings', 'geditorial-listed' ),
				'primary_taxonomy' => _n_noop( 'Listing Category', 'Listing Categories', 'geditorial-listed' ),
				'primary_subterm'  => _n_noop( 'Listing Level', 'Listing Levels', 'geditorial-listed' ),
				'program_taxonomy' => _n_noop( 'Listing Program', 'Listing Programs', 'geditorial-listed' ),
				'span_taxonomy'    => _n_noop( 'Listing Span', 'Listing Spans', 'geditorial-listed' ),
				'type_taxonomy'    => _n_noop( 'Listing Type', 'Listing Types', 'geditorial-listed' ),
				'status_taxonomy'  => _n_noop( 'Listing Status', 'Listing Statuses', 'geditorial-listed' ),
			],
			'labels' => [
				'primary_posttype' => [
					'menu_name'      => _x( 'Listings', 'Label: `menu_name`', 'geditorial-listed' ),
					'featured_image' => _x( 'Listing Poster', 'Label: Featured Image', 'geditorial-listed' ),
					'metabox_title'  => _x( 'The Listings', 'Label: MetaBox Title', 'geditorial-listed' ),
				],
				'primary_paired' => [
					'metabox_title' => _x( 'In This Listing', 'Label: MetaBox Title', 'geditorial-listed' ),
				],
				'primary_taxonomy' => [
					'menu_name' => _x( 'Categories', 'Label: Menu Name', 'geditorial-listed' ),
				],
				'program_taxonomy' => [
					'menu_name' => _x( 'Programs', 'Label: Menu Name', 'geditorial-listed' ),
				],
				'span_taxonomy' => [
					'menu_name' => _x( 'Spans', 'Label: Menu Name', 'geditorial-listed' ),
				],
				'type_taxonomy' => [
					'menu_name' => _x( 'Types', 'Label: Menu Name', 'geditorial-listed' ),
				],
				'status_taxonomy' => [
					'menu_name' => _x( 'Statuses', 'Label: Menu Name', 'geditorial-listed' ),
				],
			],
		];
	...
	protected function define_default_terms()
	{
		return [
			'primary_subterm' => [
				'preliminary'  => _x( 'Preliminary', 'Subterm Taxonomy: Default Term', 'geditorial-listed' ),
				'intermediate' => _x( 'Intermediate', 'Subterm Taxonomy: Default Term', 'geditorial-listed' ),
				'advanced'     => _x( 'Advanced', 'Subterm Taxonomy: Default Term', 'geditorial-listed' ),
				'special'      => _x( 'Special', 'Subterm Taxonomy: Default Term', 'geditorial-listed' ),
			],
			'status_taxonomy' => [
				// TODO: finish the list
				'planned' => _x( 'Planned', 'Status Taxonomy: Default Term', 'geditorial-listed' ),
				'held'    => _x( 'Held', 'Status Taxonomy: Default Term', 'geditorial-listed' ),
			],
	...
	protected function get_global_fields()
	{
		return [ 'meta' => [
			$this->constant( 'primary_posttype' ) => [
				...
				'listing_code' => [
					'title'       => _x( 'Listing Code', 'Field Title', 'geditorial-listed' ),
	...
	public function identified_default_posttype_identifier_metakey( $default, $posttype )
	{
		if ( $posttype == $this->constant( 'primary_posttype' ) )
			return Services\PostTypeFields::getPostMetaKey( 'listing_code' );
	...
	public function pairedimports_import_types( $types, $linked, $posttypes, $module_key )
	{
		...
		if ( $field = Services\PostTypeFields::isAvailable( 'listing_code', $this->constant( 'primary_posttype' ), 'meta' ) )
	...
	{
		if ( empty( $data ) || empty( $data['listing_code'] ) || $module !== 'meta' )
			return;
		...
		$this->posttypefields_connect_paired_by( 'listing_code', $data['listing_code'], $post );
	}
	...
	protected function render_imports_html( $uri, $sub )
	{
		return $this->paired_imports_render_tablelist( $uri, $sub, NULL,
			_x( 'Listing Imports', 'Header', 'geditorial-listed' ) );
	}
}
