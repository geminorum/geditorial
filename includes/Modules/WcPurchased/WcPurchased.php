...
class WcPurchased extends gEditorial\Module
{
		...
		return [
			...
			'_general' => [
				[
					...
					'title'       => _x( 'Purchased on Dashboard', 'Setting Title', 'geditorial-wc-purchased' ),
					'description' => _x( 'Displays recently purchased products on front-end account page.', 'Setting Description', 'geditorial-wc-purchased' ),
				],
				[
					'field'       => 'purchased_dashboard_title',
					'type'        => 'text',
					'title'       => _x( 'Dashboard Purchased Title', 'Setting Title', 'geditorial-wc-purchased' ),
					'description' => _x( 'Appears as title of the purchased products menu on front-end account page.', 'Setting Description', 'geditorial-wc-purchased' ),
					'placeholder' => _x( 'Purchased Products', 'Default', 'geditorial-wc-purchased' ),
				],
			],
		];
	}
	...
	public function init()
	{
		...
		if ( $this->get_setting( 'purchased_dashboard' ) ) {
			...
			// FIXME: WTF: must only append to my account page
	...
	public function account_menu_items( $items, $endpoints )
	{
		return Arraay::insert( $items, [
			$this->constant( 'endpoint_purchased' ) => $this->get_setting_fallback( 'purchased_dashboard_title', _x( 'Purchased Products', 'Default', 'geditorial-wc-purchased' ) ),
		], 'orders', 'after' );
	}
	...
	public function account_purchased_products_endpoint()
	{
		...
		if ( ! empty( $ids ) ) {
			...
			// TODO: customize via settings
			HTML::desc( _x( 'Nothing purchased yet.', 'Message', 'geditorial-wc-purchased' ) );

			$this->actions( 'account_purchased_empty', $user_id );
		}
	}
}
