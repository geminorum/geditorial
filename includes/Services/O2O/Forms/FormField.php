...
abstract class FormField implements FormFieldInterface
{
	...
	{
		if ( is_a( $args, __NAMESPACE__.'\\FormFieldInterface' ) )
			return $args;
	...
	public function render( $value = NULL )
	{
		...
		$args['name'] = API::get_name( $args['name'] );

		return str_replace( API::TOKEN, $this->_render( $args ), $this->wrap );
	}
	...
	/**
	 * Handle arguments for a single checkbox or radio input.
	 *
	 * @param array $args
	 * @return string
	 */
	public static function _checkbox( $args )
	{
		$args = wp_parse_args( $args, [
			'value'   => TRUE,
			'desc'    => NULL,
	...
	/**
	 * Generate HTML with the final arguments.
	 *
	 * @param array $args
	 * @return string
	 */
	public static function _input_gen( $args )
	{
		$args = wp_parse_args( $args, [
			'value' => NULL,
			'desc'  => NULL,
		...
		if ( 'textarea' == $args['type'] ) {
			...
			$input = Core\HTML::tag( 'textarea', $args['extra'], esc_textarea( $args['value'] ) );

		} else {

			$args['extra']['value'] = $args['value'];
			$args['extra']['type']  = $args['type'];
			$input = Core\HTML::tag( 'input', $args['extra'] );
		}
	 ...
	 * @return string
	 */
	public static function add_label( $input, $desc, $desc_pos )
	{
		if ( empty( $desc ) )
			return $input;
	 ...
	 * @return string
	 */
	public static function add_desc( $input, $desc, $desc_pos )
	{
		if ( empty( $desc ) )
			return $input;
