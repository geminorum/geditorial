...
class gEditorialContest extends gEditorialModuleCore
{
	...
	function module_defaults_meta( $default_options, $mod_data )
	{
	...
	function meta_init( $meta_module )
	{
	...
	function meta_dbx_callback( $func, $post_type )
	{
		global $gEditorial;
		if ( $post_type == $this->module->constants['contest_cpt']
			|| $post_type == $this->module->constants['apply_cpt'] )
	...
	function settings_post_types_option( $section )
	{
		global $gEditorial;
    ...
    function save_post_contest_cpt( $post_id, $post )
    {
     ...
     */
    function pre_insert_term( $term, $taxonomy )
    {
        //gtheme_dump($taxonomy); die();

        //if ( $this->_issue_tax == $taxonomy && ( ! current_user_can( 'edit_theme_options' ) ) )
        if ( $this->module->constants['contest_tax'] != $taxonomy )
            return $term;
    ...
    function save_post_apply_cpt( $post_id, $post )
    {
        if ( ( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE )
			|| empty( $_POST )
			|| $post->post_type == 'revision' )
	...
	function do_meta_box_contest_cpt( $post )
    {
		do_action( 'geditorial_the_contest_meta_box', $post );

        ?><p><input style="1width:49%;" type="number" step="1" size="4"
            name="menu_order" id="menu_order" class="small-text"
            value="<?php echo esc_attr( $post->menu_order ) ?>"
            title="<?php echo esc_attr( __( 'Order' ) ); ?>"
            placeholder="<?php echo esc_attr( __( 'Order' ) ); ?>" /></p> <?php
	...
	{
        $terms = $this->get_contests( $post->ID, true );
        do_action( 'geditorial_contest_meta_box', $post, $terms );
	}

	function geditorial_contest_meta_box( $post, $the_terms )
	{
        ...
        foreach ( $the_terms as $the_term ) {
            $contests_dropdowns[$the_term->term_id] = wp_dropdown_categories( array(
                ...
                'class' => 'geditorial_contest_term_dropdown',
                'show_count' => 1,
                'hide_empty' => 0,
                'echo' => 0,
				//'walker' => new gEditorialTermDropdownWalker,
            ) );
        ...
        foreach( $contests_dropdowns as $issues_term_id => $contests_dropdown ) {
            if ( $contests_dropdown ) {
                echo '<div class="geditorial_contest_term_wrap">';
    ...
    function get_contests( $post_ID, $object = false )
    {
        $the_terms = array();
